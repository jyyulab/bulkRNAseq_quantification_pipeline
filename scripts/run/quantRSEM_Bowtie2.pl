...
while (<INPUT>) {
    ...
    my $dir_quantRSEM = "$F[5]/$F[0]/quantRSEM_Bowtie2";
    ...
    open (OUT, "> $dir_quantRSEM/quantRSEM_Bowtie2.sh") or die;
    print OUT "#BSUB -P bulkRNAseqQuantification\n#BSUB -n 8\n#BSUB -M 4000\n#BSUB -oo $dir_quantRSEM/quantRSEM_Bowtie2.out -eo $dir_quantRSEM/quantRSEM_Bowtie2.err\n#BSUB -J quantRSEM_Bowtie2_$F[0]\n#BSUB -q superdome\n\n";
    ...
    if ($F[1] eq "PE") {
        ...
        if (-e "$dir_preProcess/fqClean.fq.gz") {
            print OUT "$dir_bin/rsem-calculate-expression --num-threads 8 --bowtie2 --bowtie2-path $dir_bin --bowtie2-sensitivity-level sensitive --strandedness $libtype --phred33-quals --sort-bam-by-coordinate $dir_preProcess/fqClean.fq.gz $F[3]/bulkRNAseq/RSEM/index_bowtie2/assembly $dir_quantRSEM/quant";
        }
        else {
    ...
    `bsub < $dir_quantRSEM/quantRSEM_Bowtie2.sh`;
