...
**Alignment statistics** is useful to tell:\
...
if (grepl("paired end", fltStat$summary$sequencing)) {n_total <- n_total / 2; n_filtered <- n_filtered / 2}
...
n_mapped <- (alnStat[1,2] + alnStat[1,3])
pct_mapped <- round((n_mapped/n_filtered * 100), 2)

n_unique <- alnStat[2,1]
...
**Quantification statistics** is used to tell how accurate the quantification results are. Specifically:\
...
pval_tr.pearson <- as.numeric(cor_tr.pearson$p.value); coef_tr.pearson <- sprintf("%.4f", as.numeric(cor_tr.pearson$estimate))

cor_tr.spearman <- cor.test(master_tr.sel$TPM, master_tr.sel$TPM_Salmon, method = "spearman", exact = T)
pval_tr.spearman <- as.numeric(cor_tr.spearman$p.value); coef_tr.spearman <- sprintf("%.4f", as.numeric(cor_tr.spearman$estimate))
...
pval_ge.pearson <- as.numeric(cor_ge.pearson$p.value); coef_ge.pearson <- sprintf("%.4f", as.numeric(cor_ge.pearson$estimate))

cor_ge.spearman <- cor.test(master_ge.sel$TPM, master_ge.sel$TPM_Salmon, method = "spearman", exact = T)
pval_ge.spearman <- as.numeric(cor_ge.spearman$p.value); coef_ge.spearman <- sprintf("%.4f", as.numeric(cor_ge.spearman$estimate))

quant_table <- matrix(c("Transcript", identTr.rsem, identTr.salmon, identTr.overlap, coef_tr.pearson, pval_tr.pearson, coef_tr.spearman, pval_tr.spearman, "Gene", identGe.rsem, identGe.salmon, identGe.overlap, coef_ge.pearson, pval_ge.pearson, coef_ge.spearman, pval_ge.spearman), nrow = 2, byrow = T)
...
**Quantification accuracy** is evaluated by the correlation of quantification results by RSEM and Salmon. **>0.85** coefficient for either **Pearson correlation** or **Spearman's rank correlation** is expected.

```{r quantification_accuracy, echo=FALSE}
p3 <- ggplot2::ggplot(master_tr.sel, aes(x = log2(TPM+1), y = log2(TPM_Salmon+1))) +
  geom_point(size = 0.5) +
  labs(x = "Log2(TPM + 1) by RSEM", y = "Log2(TPM + 1) by Salmon", title = paste0("Transcript level:\nN=", nrow(master_tr.sel), ", rho=", coef_tr.spearman, ", P-val=", pval_tr.spearman)) +
  theme(
...
p4 <- ggplot2::ggplot(master_ge.sel, aes(x = log2(TPM+1), y = log2(TPM_Salmon+1))) +
  geom_point(size = 0.5) +
  labs(x = "Log2(TPM + 1) by RSEM", y = "Log2(TPM + 1) by Salmon", title = paste0("Gene level:\nN=", nrow(master_ge.sel), ", rho=", coef_ge.spearman, ", P-val=", pval_ge.spearman)) +
  theme(
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = "white", color = "black", size = 1),
    plot.title = element_text(face = "bold"),
    legend.position = "none",
...
p5 <- ggplot(temp.df, aes(x = Bins)) + geom_density(aes(y = ..scaled..), stat = "density", fill = "red", alpha = 0.5, color = NA) +
  labs(x = "Gene Body Percentile (5'->3')", y = "Coverage", title = paste0("Mean of Coverage: ", round(pct.mean,4), "\nCoefficient of Skewness (Fisher's moment): ", round(coef.skewness,4))) +
  theme(
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = "white", color = "black", size = 1),
    plot.title = element_text(face = "bold"),
    legend.position = "none",
...
```{r genebody_coverage_plot, fig.width=5, fig.height=5, fig.align = 'center', echo=FALSE}
p5
```
